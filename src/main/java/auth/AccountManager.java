...
public class AccountManager {
    ...
    public void register(String name, String password) {
        ...
        String sql = "INSERT INTO auth_account(name, password) VALUES(? , ?)";
...
//            The parameters can be set later using the methods of the "PreparedStatement" object
    ...
    public boolean authenticate( String name, String password) {
        String sql = "SELECT name, password FROM auth_account WHERE name = ? and password = ?";
        try (PreparedStatement statement =
                     databaseConnection.getConnection().prepareStatement(sql)) {
            statement.setString(1, name);
...
//            In case, if SQL query doesn't return any rows,it will still be considered successfuland return true.
...
//            result to your SQL query and return true if the result exists or false if the result is empty.
    ...
    public Account getAccount(String name){
        String sql = "SELECT password FROM auth_account WHERE name = ? ";
        try (PreparedStatement statement =
                     databaseConnection.getConnection().prepareStatement(sql)) {
            statement.setString(1, name);
